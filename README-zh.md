...
### v0.9.5 (2026-09-23)
> 🔴 播放与目录功能失效 + 🟠 后端逻辑 + ⚡ 效率专项
- 🔴 **DSD 不再播成噪声** — 解码器未启用 PCM 抽取模式，输出的是原始 DSD 比特流（2.8224MHz），表现为 8 倍速刺耳噪声
- 🔴 **外接盘未挂载时不再覆盖音乐文件夹配置** — 此前会把不可达路径当作「未配置」并写回默认目录，原配置不可恢复
- 🔴 **删除二级文件夹恢复正常** — 原逻辑要求链接的外部目标位于主文件夹内，而二级文件夹本就是指向外部，导致每次删除必然失败
- 🔴 **首次启动自动显示扫描结果** — 扫描完成后广播事件，界面不再停在「曲库为空」
- 🟠 **扫描实现统一** + 顺带修复「扫描提示永远显示 0 首歌曲」
- 🟠 **元数据损坏的文件不再被丢弃**（改用文件名兜底入库）、**多声道文件不再被当作 0 秒跳过**
- 🟠 **嵌套符号链接不再产生「幽灵歌」**（列表可见但操作全被拒）
- 🟠 **取消隐藏加密歌曲后不再被重新隐藏**（新增 `unhidden_overrides` 表记住用户意愿）
- ⚡ **扫描提速 85%** — 去重从逐文件 `realpath` 改为一次 `stat` 取 `(dev, ino)`：6000 首实测 824ms → 121ms
- ⚡ **列表封面改用 56px 并批量取图**（单图像素量降至 1/12.7、每屏 N 次 IPC 压到 1 次）；**首屏懒加载**（入口包 -8.7%）
- ⚡ **修掉 4 处在此前修复过程中引入的性能回归** — 白名单递归查询无缓存（20ms/次）、批量校验重复解析目录、缩略图回收重复 stat 源文件、取消隐藏逐条写库
- 🐛 **点赞不再清空列表多选**
- 📈 **测试增长** — 后端 88→116，前端 155→157

### v0.9.4 (2026-09-20)
> 🔒 数据安全 + 🐛 扫描正确性 + 🎵 歌词 + 🖼️ 资源占用
- 🔒 **外接盘未挂载时不再误删用户数据** — 清理逻辑会删除所有 `!exists()` 的记录，经 FK 级联连带删掉喜欢/播放历史/播放次数；盘未挂载时整盘用户数据会被永久删除。新增护栏：仅当「文件缺失且**父目录仍存在**」才判定为真实删除
- 🐛 **增量扫描同时校验 file_size** — 仅比 mtime 会漏判 `cp -p`/`rsync -t`/恢复备份等「内容变但 mtime 不变」的场景，元数据永久陈旧
- 🐛 **隐藏目录作扫描根不再被整体过滤** — 音乐文件夹名为 `.music` 时曾静默返回 0 首
- 🐛 **非 UTF-8 文件名不再入库** — `to_string_lossy` 产生的 U+FFFD 路径永远无法打开原文件
- 🎵 **`.lrc` 符号链接越权读取修复** + **UTF-8 歌词不再被误判致乱码**
- 🖼️ **嵌入封面上限 5MB** + **缩略图孤儿缓存回收**（此前随库变更持续累积）
- 📈 **测试增长** — 后端 72→75，前端 155
